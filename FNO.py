...
@author: vgopakum
FNO modelled over the MHD data built using JOREK for a single isothermal diffusion of a plasma blob. 
"""
# %%
...
run = Run()
run.init(folder="/FNO_MHD", tags=['FNO', 'MHD', 'JOREK', 'Single-Blob', 'Isothermal'], metadata=configuration)
...
if field == 'Phi':
    u_sol = data['Phi'][:,1:,:,:].astype(np.float32) / 1e5
# elif field == 'w':
#     u_sol = data['w'][:,1:,:,:].astype(np.float32) / 1e1
elif field == 'rho':
...
# %%
#Saving the Model
model_loc = file_loc + '/Models/FNO_isothermal_blob_' + run.name + '.pth'
...
# %%
output_plot = file_loc + '/Plots/IsothermalBlobs_' + configuration['Field'] + '_' + run.name + '.png'
