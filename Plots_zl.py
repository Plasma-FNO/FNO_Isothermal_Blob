...
model.load_state_dict(torch.load(file_loc + '/Models/FNO_multi_blobs_forward-canal.pth', map_location=torch.device('cpu'))) #250 Epochs 
...
plt.plot(np.arange(T_in, T_in + T), (err_rho+err_phi), label='Cumulative', alpha=0.8,  color = 'black', ls='--', linewidth=5)
plt.legend()
plt.grid()
...
for field in dims:  
    ...
    if field == 'Phi':
        u_sol = data['Phi'].astype(np.float32)   / 1e5
    elif field == 'T':
        u_sol = data['T'].astype(np.float32)     / 1e6
    elif field == 'rho':
        u_sol = data['rho'].astype(np.float32)   / 1e20
    ...
    if field == 'rho':
        model.load_state_dict(torch.load(file_loc + '/Models/FNO_multi_blobs_navy-sectional.pth', map_location=torch.device('cpu')))
    if field == 'Phi':
        model.load_state_dict(torch.load(file_loc + '/Models/FNO_multi_blobs_figurative-dolce.pth', map_location=torch.device('cpu')))
    model.to(device)
...
plt.savefig("isothermal-blob_error_growth.pdf", bbox_inches='tight')
plt.savefig("isothermal-blob_error_growth.svg", bbox_inches='tight')
# %%
